        ...
        fetch("getAllOrderDetail2")
            .then(function (response) {
                ...
                response.json().then(function (data) {
                    ...
                    console.log("這是訂單明細資料", OrderDetailData2);
                    getOrderDetail();
                });
            })
            .catch(function (err) {
...
//
    ...
    function select4edit() {
        ...
        const OrderDetaillwt = new Set();
        const OrderDetailDatas = new Set();
        let newstring = ""
        OrderDetailData2.forEach(function (row) {
            ...
            OrderDetaillwt.add(newstring);
            OrderDetailDatas.add(orderDetailId);
        })

        for (i = 0; i < OrderDetailData.length; i++) {
            const row1 = OrderDetaillwt[i];
            const row2 = OrderDetailDatas[i];

            const option = new Option(row2, row1);
