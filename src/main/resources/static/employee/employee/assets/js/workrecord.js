            ...
            .then(function (response) {
                ...
                response.json().then(function (data) {
                    ...
                    for (let i = 0; i < WorkRecordData.length; i++) {
                        ...
                        const recordDate = dateformat(new Date(row.recordDate));
                        const performanceScore = row.performanceScore;
                        ...
                        <div class="mb-3">
                         ...
                         <div class="mb-3">
                                            <label for="employeeId${i}"
                                                   class="col-form-label">員工姓名:</label>
                                            <select id="employeeId${i}">
                                                <option value="">請選擇</option>
                                                <option value="7">詹凱程</option>
                                                <option value="8">賴俊吉</option>
                                                <option value="9">李天鈞</option>
                                                <option value="10">黃國龍</option>
                                                <option value="13">黃氏慧</option>
                                                <option value="14">杜昶成</option>
                                                <option value="15">吳佳鴻</option>
                                                <option value="16">詹浩宇</option>
                                                <option value="17">阮光興</option>
                                                <option value="18">阮文福</option>
                                                <option value="19">黎春寶</option>
                                                <option value="20">寧德孟</option>
                                                <option value="21">鄧武協</option>
                                                <option value="22">林子翔</option>
                                                <option value="25">李世園</option>
...
<!--                                                <option value="31"></option>-->
                       ...
                       <div class="mb-3">
                            <label For="recordDate${i}"
                                   class="col-form-label">日期:</label>
                            <input type="date" class="form-control" id="recordDate${i}" value="${recordDate}">
                        </div>
                         <div class="mb-3">
                            <label For="performanceScore${i}"
                                   class="col-form-label">績效:</label>
                            <input type="text" class="form-control" id="performanceScore${i}" value="${performanceScore}">
                        </div>
                         <div class="mb-3">
                            <label For="additionalDetails${i}"
                                   class="col-form-label">備註:</label>
                            <input type="text" class="form-control" id="additionalDetails${i}" value="${additionalDetails}">
                        </div>
                    </form>
                </div>
...
// 自動選好修改內的select
    ...
    function editOrder(i) {
        ...
        const recordDate = document.getElementById(`recordDate${i}`).value;
        const performanceScore = document.getElementById(`performanceScore${i}`).value;
        const additionalDetails = document.getElementById(`additionalDetails${i}`).value;
...
//     // ============================   newOrder()新增訂單========================
    function newOrder() {
        const employeeId = document.querySelector('#employeeId4new').value;
        const recordDate = document.querySelector('#recordDate4new').value;
        const additionalDetails = document.querySelector('#additionalDetails4new').value;
...
//
    ...
    function setdeletebutton() {
        for (let i = 0; i < dataaccount; i++) {
            const workRecordId = WorkRecordData[i].workRecordId;
            const deletebutton = document.getElementById('delete' + i);
            deletebutton?.addEventListener('click', () => {
                console.log('第' + i + '個的ｉｄ:' + workRecordId)
                deledtbyPK(workRecordId);
            })
        }
    }

    // ============================9. 刪除方法========================
    function deledtbyPK(workRecordId) {
        const confirmed = confirm("確定要刪除嗎？");
        console.log(workRecordId)
        if (confirmed) {
            fetch('deleteWorkRecord', {
                method: 'POST', headers: {
                    'Content-Type': 'application/json',
